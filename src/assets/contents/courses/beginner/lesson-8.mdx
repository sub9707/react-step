...
<Image
  src="/content/beginner/8/useEffect-patterns.png"
  alt="useEffect 패턴들"
  width="half"
  caption="useEffect의 다양한 실행 패턴"
/>
...
<CodeSandbox
title="useEffect 의존성 배열 비교"
code={`import { useEffect, useState } from 'react';

function EffectDependencyExample() {
  const [count, setCount] = useState(0);
  const [name, setName] = useState('React');

  // 1. 의존성 배열 없음 - 매번 실행
  useEffect(() => {
    console.log('1. 매번 실행됩니다');
  });

  // 2. 빈 배열 - 최초 한 번만
  useEffect(() => {
    console.log('2. 최초 한 번만 실행됩니다');
  }, []);

  // 3. count가 변경될 때만
  useEffect(() => {
    console.log('3. count가 변경됨:', count);
  }, [count]);

  // 4. name이 변경될 때만
  useEffect(() => {
    console.log('4. name이 변경됨:', name);
  }, [name]);
...
function App() {
  return <EffectDependencyExample />;
}`}
/>
...
때로는 데이터를 주기적으로 업데이트해야 하는 경우가 있습니다. 예를 들어, 실시간 채팅이나 주식 가격 같은 데이터입니다.

<CodeSandbox
title="실시간 데이터 업데이트 예제"
...
function RealTimeData() {
  ...
  const fetchData = async () => {
    try {
      // 실제로는 실시간 API를 호출하지만, 여기서는 시뮬레이션
      const mockData = {
  ...
  useEffect(() => {
    ...
    if (isAutoUpdate) {
      interval = setInterval(fetchData, 2000); // 2초마다 업데이트
    }
    ...
    <div style={{ padding: '20px', maxWidth: '400px', margin: '0 auto' }}>
      ...
      {data && (
        <div style={{ 
          ...
          <div style={{ 
            color: data.change >= 0 ? '#27ae60' : '#e74c3c',
            fontSize: '16px',
            margin: '10px 0'
          }}>
      ...
      <div style={{ marginTop: '20px', textAlign: 'center' }}>
        <button
          ...
          style={{
            padding: '10px 20px',
            backgroundColor: isAutoUpdate ? '#e74c3c' : '#27ae60',
            color: 'white',
            border: 'none',
            borderRadius: '5px',
            cursor: 'pointer',
        ...
        >
          {isAutoUpdate ? '자동 업데이트 중지' : '자동 업데이트 시작'}
        </button>
